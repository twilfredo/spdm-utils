...
/// This header is defined in QEMU and it extends the spdm_storage_transport_virtual_header_t
...
fn qemu_storage_decode_transport_header(
    ...
    // Alias the Qemu header into spdm_storage_transport_virtual_header_t
    ...
    // starts with spdm_storage_transport_virtual_header_t and can directly be
    ...
    qemu_length = if cmd == SOCKET_SPDM_STORAGE_IF_SEND {
        qemu_length += std::mem::size_of::<spdm_storage_transport_virtual_header_t>() as u32;
        qemu_length
    } else {
        qemu_length
    };
