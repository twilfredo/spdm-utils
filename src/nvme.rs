...
impl NvmeSecSendCmds {
    ...
    pub fn gen_libspdm_encoded_sec_send_args(
        ...
        let trans_hdr =
            unsafe { *(dptr.as_mut_ptr() as *mut spdm_storage_transport_virtual_header_t) };
        assert_eq!(
        ...
        let transfer_len = msg_len - LIBSPDM_STORAGE_TRANSPORT_HEADER_SIZE as usize;
        if transfer_len <= 0 {
            return Err(Errno::ENOMSG);
        }
...
impl NvmeSecRecvCmds {
    ...
    pub fn gen_libspdm_encoded_sec_recv_args(
        ...
        let trans_hdr = unsafe {
            *(transport_message as *const u8 as *mut spdm_storage_transport_virtual_header_t)
        };
        assert_eq!(
            trans_hdr.security_protocol,
            u8::from(SpcSecurityProtocols::DmtfSpdm)
